<template>
  <ReceiptBlock :class="CSS_NAME">
    ...
    <template #default="{ className }">
      ...
      <slot name="after" :class-name="[`${CSS_NAME}__row`, className]" :footer-class-name="[`${CSS_NAME}__footer`, className]" />
    </template>
  </ReceiptBlock>
</template>
...
@include component($prefix) {
  ...
  @include element('body') {
    border-bottom: get-var(border-bottom, 0, $prefix: $prefix) solid get-var(color-light);
